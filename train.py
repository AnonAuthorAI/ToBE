...
def train(args):
    ...
    # decide the ToBE mode that will be tested
    if args.tobe_mode in MODE_SPACE:
        mode_space = [args.tobe_mode]
    elif args.tobe_mode == "all":
        mode_space = MODE_SPACE
    else:
        raise ValueError(
            f"tobe_mode must be one of {MODE_SPACE + ['all']}, got {args.tobe_mode}."
        )

    # run the experiment
    for tobe_mode in mode_space:
        print_centered(
            f"Dataset [{args.dataset.title()}] - {args.imb_type.title()}IR [{args.imb_ratio}] - ToBE [{tobe_mode}]",
            width=80,
            fillchar="=",
            prefix="\n",
        )
        ...
        for i_run in range(1, args.n_runs + 1):
            ...
            # tobe augmenter
            augmenter = TopoBalanceAugmenter(mode=tobe_mode, random_state=seed)
            # trainer
            trainer = NodeClassificationTrainer(
                model=model,
                data=data,
                device=device,
                augmenter=augmenter,  # ToBE augmentation, to disable, set augmenter=None
            ...
            # train the GNN with ToBE augmentation
